...
import no.tleirvik.model.filemanagement.decoders.Decoder;
import no.tleirvik.model.filemanagement.decoders.Life105Decoder;
import no.tleirvik.model.filemanagement.decoders.Life106Decoder;
import no.tleirvik.model.filemanagement.decoders.RLEDecoder;
import no.tleirvik.model.gameoflife.MetaData;
import no.tleirvik.model.gameoflife.PatternFormatException;
...
import no.tleirvik.model.util.DialogBoxes;
...
public class FileLoader {
     ...
     */
    public boolean loadBoard(File f, no.tleirvik.model.filemanagement.EncodeType type) {
        if (!f.canRead()) {
     ...
     * @return true if the board could be loaded. Otherwise false
     */
    public boolean loadBoardFromURL(URL url, no.tleirvik.model.filemanagement.EncodeType type) {
        URLConnection conn;
        try {
     ...
     */
    private void decodeBoard(BufferedReader reader, no.tleirvik.model.filemanagement.EncodeType type) throws IOException, PatternFormatException {
        switch(type) {
            case RLE:
